...
def _bubble(event: rx.Var[MeetingEvent]) -> rx.Component:
    ...
    avatar = rx.box(
        ...
        rx.box(
            rx.icon(icon, size=12, color=COLORS["surface"]),
            position="absolute",
            bottom="-4px",
            right="-4px",
            padding="3px",
    ...
    return rx.hstack(
        ...
        width="100%",
        min_width="0",
        # Cosmetic entrance animation (fade + slide-up) as each streamed bubble
        # lands; the class lets the reduce-motion media query disable it.
        class_name="saisei-bubble-in",
        style=BUBBLE_IN,
    )


def _pending_indicator() -> rx.Component:
    """Transient 'persona is considering…' row shown before a bubble lands.

    Driven by ``SaiseiUIState.pending_speaker`` (set just before each paced
    bubble). Shows the upcoming persona's avatar monogram + an animated
    three-dot indicator so the meeting feels turn-by-turn. Renders nothing when
    no speaker is pending (e.g. pacing disabled).
    """
    speaker = SaiseiUIState.pending_speaker
    accent = rx.match(
        speaker,
        *[(k, p.accent) for k, p in PERSONAS.items()],
        COLORS["text_muted"],
    ...
    dot = lambda delay: rx.box(  # noqa: E731 - tiny inline style helper
        width="6px",
        height="6px",
        border_radius=RADII["pill"],
        background=accent,
        style={"animation": f"saisei-dots 1.2s {delay} infinite ease-in-out"},
    )
    return rx.cond(
        speaker != "",
        rx.hstack(
            ...
            rx.hstack(
                rx.text(name_ja, size="1", color=COLORS["text_muted"]),
                rx.hstack(dot("0s"), dot("0.2s"), dot("0.4s"), spacing="1"),
                align="center",
                spacing="2",
            ),
            align="center",
...
def _action_bar() -> rx.Component:
    ...
    return rx.cond(
        ...
        rx.vstack(
            rx.divider(),
            rx.hstack(
                rx.icon("gavel", size=16, color=COLORS["chrome"]),
                rx.text(
            ...
            padding="14px",
            background=COLORS["surface"],
            border=f"1px solid {COLORS['chrome']}55",
            border_radius=RADII["md"],
...
def meeting_panel() -> rx.Component:
    ...
    return rx.vstack(
        rx.hstack(
            rx.icon("messages-square", size=18, color=COLORS["chrome"]),
        ...
        rx.text(
            "各発言はリハーサルです。「【未検証 / unverified】」と付いた記述は根拠に"
            "紐づかない参考意見です。 (Voices are a rehearsal; text tagged "
            "【未検証 / unverified】 is ungrounded model commentary.)",
            style=TYPE["small"],
            color=COLORS["text_faint"],
        ),
        rx.divider(),
