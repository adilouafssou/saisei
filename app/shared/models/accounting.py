...
class TrialBalance(BaseModel):
    ...
    # ``extra="ignore"`` (not "forbid") so the model round-trips through its own
    # ``model_dump()`` output: the computed fields below (uriage_sourieki,
    # eigyo_rieki, keijo_rieki) are emitted by ``model_dump`` but have no
    # setter, so re-validating that dict under "forbid" raised extra_forbidden.
    # ``SaiseiState.shisanhyo`` is delivered as serialized dicts over the
    # LangGraph checkpoint, so the serialized form must validate cleanly.
    ...
    eigai_shueki: JPY = Field(default=0, description="Non-operating income / 営業外収益 (JPY).")
    eigai_hiyo: JPY = Field(default=0, description="Non-operating expenses / 営業外費用 (JPY).")
