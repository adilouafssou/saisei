...
(正常) FSA classification, aligned with FSA guidelines.
...
2. FSA classification path: current classification must not be YUKYO_GUCHI
...
def guarantor_critic_node(
    ...
    rationale = (
        f"信用保証協会評価: {status}. "
        + (
            f"致命的ブロッカー {len(blockers)} 件（コンプライアンス最優先）。"
            if blockers
