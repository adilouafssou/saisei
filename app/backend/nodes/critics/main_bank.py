...
_PERSONA = "main_bank"
_PRIORITY = "P1"
_PROMPT = "critic_main_bank.md"


def main_bank_critic_node(
    ...
    if not state.yakuin_hoshu_cut:
        blockers.append(
            "yakuin_hoshu_not_cut: 役員報酬削減コミットメント（yakuin_hoshu_cut）が"
            "確認されていません。"
            "主幹事銀行は役員報酬の削減コミットメントなしに支援を承認しません。"
            "担当者がHITL割り込みで yakuin_hoshu_cut=True を設定してください。"
        )
    ...
    if deficit and not state.personal_asset_disposal:
        blockers.append(
            "no_asset_disposal: 資金繰り赤字にもかかわらず、個人資産処分コミットメント"
            "（personal_asset_disposal）が確認されていません。"
            "担当者がHITL割り込みで personal_asset_disposal=True を設定してください。"
        )
    ...
    rationale = (
        ...
        + (
            ...
            if blockers
            else "役員報酬削減および資産処分コミットメントを確認しました。"
        )
    )
