...
(pro-rata / zandaka pro-rata, Meishoritsushugi / 名称率主義) within a defined
tolerance.
...
def sub_bank_critic_node(
    state: SaiseiState, settings: Settings | None = None
) -> dict[str, Any]:
    ...
    if not strategies:
        blockers.append(
            "no_strategies: 改善施策が提案されていません。"
            "協調融資銀行は負担分担を評価できません。"
        )
        status = "FAIL"
        rationale = "協調融資銀行評価: FAIL. 施策なし。"
        feedback = CriticFeedback(
            ...
            simulated_argument=simulate_persona_argument(
                _PROMPT, _PERSONA, status, blockers, rationale, settings=settings
            ),
        )
    ...
    if not stake_based and total_uplift == 0:
        ...
        status = "PASS"
        rationale = "協調融資銀行評価: PASS. 期待改善額がゼロのため負担分担評価をスキップします。"
        feedback = CriticFeedback(
            persona=_PERSONA,
            status=status,
            fatal_blockers=[],
            ...
            rationale=rationale,
            simulated_argument=simulate_persona_argument(
                _PROMPT, _PERSONA, status, [], rationale, settings=settings
            ),
        )
    ...
    mode_label = "実残高ベース" if mode == "stake_based" else "ヒューリスティック代理指標"
    ...
    if main_bank_share > upper_bound:
        ...
        blockers.append(
            ...
            "協調融資銀行との負担分担を均等化してください（名称率主義 / Meishoritsushugi）。"
        )
    ...
    rationale = (
        f"協調融資銀行評価: {status}. "
        f"主幹事銀行負担比率: {main_bank_share:.1%}（{mode_label}）. "
        + (
            f"致命的ブロッカー {len(blockers)} 件。"
            if blockers
            else "負担分担は許容範囲内です。"
        )
    )
