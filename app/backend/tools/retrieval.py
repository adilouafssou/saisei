...
class RetrievalSnippet(BaseModel):
    ...
    score: float = Field(
        default=0.0, description="Similarity score (higher = more relevant)."
    )
...
class RediSearchShortTermMemory:
    ...
    def _query_redisearch(self, query: str, top_k: int) -> list[RetrievalSnippet]:
        ...
        _log.info(
            "memory.shortterm.redisearch.search", chars=len(query), hits=len(hits)
        )
        return hits
    ...
    def _index_redisearch(self, query: str, snippets: list[RetrievalSnippet]) -> None:
        ...
        payload: dict[str, Any] = {
            ...
            "documents": [
                {"text": snip.text, "score": snip.score, "source": snip.source}
