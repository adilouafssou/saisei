...
def test_lead_arranger_ignores_simulated_argument() -> None:
    ...
    def _run(feedbacks: list[dict[str, Any]]) -> dict[str, Any]:
        state = SaiseiState(
            tdb_code="1234567",
    ...
    # meeting_briefing INTENTIONALLY renders each persona's simulated_argument
    # (the advisory rehearsal — see test_part4_meeting_briefing.py::
    # test_advisory_changes_only_the_briefing_text). The spine contract is that
    # NOTHING ELSE depends on it: compare all keys except the advisory briefing.
    plain_rest = {k: v for k, v in result_plain.items() if k != "meeting_briefing"}
    with_rest = {k: v for k, v in result_with_args.items() if k != "meeting_briefing"}
