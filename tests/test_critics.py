...
from app.backend.nodes.critics.sub_bank import compute_main_bank_share, sub_bank_critic_node
...
def _distressed_state_with_strategies(
    ...
    fsa_classification: FsaClass = FsaClass.YUKYO_GUCHI,
...
def test_main_bank_deterministic() -> None:
    ...
    state = _distressed_state_with_strategies(
        yakuin_hoshu_cut=True, personal_asset_disposal=True
    )
...
def test_guarantor_fail_no_strategies_doubtful() -> None:
    """YUKYO_GUCHI with no strategies → FAIL (no_recovery_plan)."""
    state = _distressed_state_with_strategies(
        strategies=[],
        fsa_classification=FsaClass.YUKYO_GUCHI,
        ews_score=75.0,
    )
    result = guarantor_critic_node(state)
    fb = result["critic_feedbacks"][0]
...
def test_guarantor_pass_low_ews_single_strategy() -> None:
    ...
    state = _distressed_state_with_strategies(
        ...
        fsa_classification=FsaClass.YOI_KANRI,
    )
    result = guarantor_critic_node(state)
    fb = result["critic_feedbacks"][0]
...
def test_lead_arranger_any_fail_rejected() -> None:
    ...
    feedbacks = [
        _make_feedback("guarantor", "PASS", "P0"),
        _make_feedback("main_bank", "FAIL", "P1", ["yakuin_hoshu_not_cut: test"]),
        _make_feedback("sub_bank", "PASS", "P2"),
    ]
    state = _state_with_feedbacks(feedbacks)
    result = lead_arranger_node(state)
    ...
    assert "yakuin_hoshu_not_cut" in result["revision_directive"]
...
def test_strategist_node_resets_critic_feedbacks() -> None:
    ...
    state = SaiseiState(
        ...
        shisanhyo=[_tb()],
        critic_feedbacks=[
            {"persona": "guarantor", "status": "PASS", "fatal_blockers": [],
             "priority": "P0", "rationale": "old"},
            {"persona": "main_bank", "status": "FAIL", "fatal_blockers": ["stale"],
             "priority": "P1", "rationale": "old"},
        ],
    )
...
def test_cyclic_revision_no_cross_round_accumulation() -> None:
    ...
    round1_feedbacks = [
        _make_feedback("guarantor", "PASS", "P0"),
        _make_feedback("main_bank", "FAIL", "P1", ["yakuin_hoshu_not_cut: round1"]),
        _make_feedback("sub_bank", "PASS", "P2"),
    ]
    ...
    round2_feedbacks_after_reset: list[dict] = []
    for fb in [
        _make_feedback("guarantor", "PASS", "P0"),
        _make_feedback("main_bank", "PASS", "P1"),  # now passes (flags set)
        _make_feedback("sub_bank", "PASS", "P2"),
    ]:
        round2_feedbacks_after_reset = critic_feedbacks_reducer(
            round2_feedbacks_after_reset, [fb]
        )
...
def test_lead_arranger_needs_human_when_only_banker_blockers() -> None:
    ...
    feedbacks = [
        _make_feedback("guarantor", "PASS", "P0"),
        _make_feedback(
            "main_bank", "FAIL", "P1", ["yakuin_hoshu_not_cut: confirm exec-comp cut"]
        ),
        _make_feedback("sub_bank", "PASS", "P2"),
    ]
    state = _state_with_feedbacks(feedbacks, revision_count=0)
...
def test_lead_arranger_rejected_when_strategist_fixable_blocker_present() -> None:
    ...
    feedbacks = [
        _make_feedback(
            "guarantor", "FAIL", "P0", ["recovery_horizon_exceeded: add strategies"]
        ),
        _make_feedback(
            "main_bank", "FAIL", "P1", ["yakuin_hoshu_not_cut: confirm exec-comp cut"]
        ),
...
def test_guarantor_uses_shared_ews_doubtful_constant() -> None:
    ...
    assert not any("insufficient_strategies" in b for b in fb_below["fatal_blockers"]), (
        f"EWS={EWS_DOUBTFUL - 0.1} (below threshold) must NOT trigger "
        f"insufficient_strategies"
    )
...
def test_guarantor_uses_shared_min_recovery_horizon_years() -> None:
    ...
    # _tb() keijo_rieki = uriage - uriage_genka - hanbaihi = 122M - 129M - 23.5M = -30.5M/mo
    # annual_deficit = 30_500_000 * 12 = 366_000_000
    # required_annual_uplift = 366_000_000 / 5 = 73_200_000
    annual_deficit = abs((122_000_000 - 129_000_000 - 23_500_000) * 12)
