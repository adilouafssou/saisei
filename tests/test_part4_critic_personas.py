...
def test_gate_verdicts_unchanged_offline() -> None:
    ...
    assert main_bank_critic_node(passing, settings=_OFFLINE)["critic_feedbacks"][0][
        "status"
    ] == "PASS"
    assert sub_bank_critic_node(passing, settings=_OFFLINE)["critic_feedbacks"][0][
        "status"
    ] == "PASS"
    assert guarantor_critic_node(passing, settings=_OFFLINE)["critic_feedbacks"][0][
        "status"
    ] == "PASS"
